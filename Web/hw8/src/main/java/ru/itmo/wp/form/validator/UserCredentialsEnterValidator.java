...
public class UserCredentialsEnterValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        if (!errors.hasErrors()) {
            ...
            if (userService.findByLoginAndPassword(enterForm.getLogin(),
                    enterForm.getPassword()) == null) {
                errors.rejectValue(
                        "password", "password.invalid-login-or-password", "Invalid login or password");
            } else if (user.getDisabled()) {
                errors.rejectValue("login", "login.try-to-enter-disable-user", "User is disabled");
            }
        }
    }
}
