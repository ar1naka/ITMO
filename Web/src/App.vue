<template>
    <div id="app">
        ...
        <Footer :posts_num="Object.keys(posts).length" :users_num="Object.keys(users).length"/>
    </div>
</template>
    ...
    beforeCreate() {
        ...
        this.$root.$on("onRegister", (login, name) => {
          if (login.length < 3 || login.length > 16){
            this.$root.$emit("onRegisterValidationError", "Wrong length of login!")
            return;
          }
          const users = Object.values(this.users).filter(u => u.login === login);
          if (users.length !== 0) {
          ...
          if (name.length < 1 || name.length > 32){
            this.$root.$emit("onRegisterValidationError", "Wrong length of name!")
          }
