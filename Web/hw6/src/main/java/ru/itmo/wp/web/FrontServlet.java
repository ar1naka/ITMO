...
public class FrontServlet extends HttpServlet {
    ...
    private Method findMethod(Class<?> pageClass, Method m, String action){
        for (Class<?> clazz = pageClass; m == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                m = clazz.getDeclaredMethod(action, HttpServletRequest.class, Map.class);
    ...
    private void get(HttpServletRequest request, Class<?> pageClass, Object page, Map<String, Object> view, Method meth) throws ServletException {
        if (meth != null) {
            try {
                meth.setAccessible(true);
                meth.invoke(page, request, view);
            } catch (IllegalAccessException | InvocationTargetException e) {
                throw new ServletException("Unable to run action [pageClass=" + pageClass + ", method=" + meth + "].", e);
            }
        }
    }
    ...
    private void process(Route route,
        ...
        Method m = null;
        m = findMethod(pageClass, m, "before");
        get(request, pageClass, page, view, m);
        ...
        m = findMethod(pageClass, m, "after");
        get(request, pageClass, page, view, m);
